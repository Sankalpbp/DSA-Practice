public class Insertion {
    ...
    public static void print ( ) {
        ...
        System.out.println ( "Adding after 3rd node: " );
        head = insertAfterTth ( head, 3, 66 );
        LinkedList.printList ( head );

    }

    private static Node insertAfterTth ( Node head, int t, int x ) {
        Node newNode = new Node ( x, null );
        Node ptr = head;

        int position = 0;

        while ( ptr != null && position < t ) {
            ptr = ptr.next;
            ++position;
        }
